...
### 5-2. 배포 후 백업 정책 (Mandatory Backup) - Ver 5.9 업데이트
...
|------|------|
| **제목 형식** | `Ver X.X.X - [기능명]` |
| **예시** | `Ver 5.9.0 - Daily Reports` |
| **브랜치** | `v4.3.0-release` (릴리스 브랜치) |

**커밋 명령어**:
```bash
git add -A && git commit -m "Ver X.X.X - [기능명]" && git push
```

#### 백업 규칙
| 항목 | 규칙 |
|------|------|
| **폴더 위치** | `/home/blue/blue/my_project/money/backup/` |
| **폴더명 형식** | `Ver_X.X.X_[기능명]` (Git 커밋 제목과 동일, 공백은 `_`로 대체) |
| **예시** | `Ver_5.9.0_Daily_Reports` |
| **제외 항목** | `venv/`, `node_modules/`, `.git/`, `__pycache__/`, `backup/` |

**백업 명령어**:
```bash
rsync -av --exclude='venv' --exclude='node_modules' --exclude='.git' --exclude='__pycache__' --exclude='backup' \
  /home/blue/blue/my_project/money/ \
  "/home/blue/blue/my_project/money/backup/Ver_X.X.X_기능명/"
```

#### 전체 배포 + 백업 워크플로우
```bash
# 1. 버전 업데이트 (App.jsx, package.json)
# 2. Git 커밋 & 푸시
cd /home/blue/blue/my_project/money
git add -A && git commit -m "Ver 5.9.0 - Daily Reports" && git push

# 3. 백업 (venv 제외)
rsync -av --exclude='venv' --exclude='node_modules' --exclude='.git' \
  ./ backup/Ver_5.9.0_Daily_Reports/
