...
def verify_signal():
    ...
    print("Fetching SOXS 5m data (1mo, prepost=True)...")
    df = yf.download("SOXS", period="1mo", interval="5m", prepost=True, progress=False)
    ...
    # Specific check for 2026-01-06 04:45
    print("\n--- Checking Data around 2026-01-06 04:45 (NY Time) ---")
    target_start = ny_tz.localize(datetime(2026, 1, 6, 4, 30, 0)) 
    target_end = ny_tz.localize(datetime(2026, 1, 6, 5, 0, 0))   
    
    subset = df[(df.index >= target_start) & (df.index <= target_end)]
    if not subset.empty:
        print(subset[['Close', 'SMA10', 'SMA30']])
    else:
        print("No data found between 04:30 and 05:00 NY")
