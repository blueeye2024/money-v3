...
def migrate_db():
    ...
    try:
        ...
        # NOTE: User calls it "journal", but actual table is likely "trade_journal" based on 'SHOW TABLES'
        ...
        if 'emotion' in columns:
            ...
            cursor.execute(f"ALTER TABLE {TARGET_TABLE} DROP COLUMN emotion")
        ...
        if 'total_assets' not in columns:
            ...
            cursor.execute(f"ALTER TABLE {TARGET_TABLE} ADD COLUMN total_assets DECIMAL(15, 2) COMMENT '총 자산 (USD)' AFTER ticker")
