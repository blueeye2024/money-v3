...
- **버전 및 빌드 시간 표시**: 웹사이트 하단(Footer)에 현재 버전과 마지막 업데이트 시간(Build Time)을 자동으로 표시.
  - 예: `Ver 3.5.0 (Updated: 2026-01-06 17:10:00)`
- **배포 프로세스 개선**: 배포 시점의 시간을 환경 변수로 주입하여 프론트엔드에 반영하는 자동화 로직 추가 (`deploy.sh`).
...
### UI 변경 (UI Changes)
- **로딩 UI 롤백**:
  - 사용자 피드백을 반영하여 Skeleton UI 대신 기존의 **애니메이션 로딩 바**를 다시 적용.
  - 시스템 가동 중임을 명확히 시각화.

## [Ver 3.0.17] - 2026-01-05

### 최적화 (Optimization)
- **Lazy Loading 적용**: 
  - 대시보드 진입 시 전체 로딩 화면 대신 **스켈레톤(Skeleton UI)**를 즉시 렌더링하도록 변경.
  - 데이터 로딩 중에도 UI 구조를 유지하여 체감 속도 향상.
- **데이터베이스 최적화**:
  - `SOXL`, `SOXS`, `UPRO` (Core Tickers) 외의 불필요한 장기 차트 데이터 삭제 및 저장 차단.

## [Ver 3.0.16] - 2026-01-05

### 성능 개선 (Performance)
- **대시보드 로딩 속도 획기적 단축**:
  - 백엔드 캐싱 전략 변경: 사용자 요청 시에는 캐시된 데이터를 즉시 반환 (`Cache Hit` 100% 지향)
  - 백그라운드 스케줄러(`monitor_signals`)가 1분마다 데이터를 사전에 강제 갱신(`Prefetching`)
  - 대기 시간(Latency) 제거 (기존: 1~3초 -> 개선: <50ms)
