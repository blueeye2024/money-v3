...
### API 조회
```bash
curl http://localhost:9100/api/v2/status/SOXL | jq '.market_info'
```
...
### 가격이 업데이트되지 않음
- SQL 실행 결과 확인 (affected rows)
- `updated_at` 컬럼 확인
- 캐시 문제: 브라우저 새로고침 (Ctrl+Shift+R)
...
- `frontend/src/components/V2SignalStatus.jsx`: 신호 비교 로직
